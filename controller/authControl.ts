...
export const verifyEmail = async (req: Request, res: Response): Promise<any> => {
  ...
  const token = (req.query.token as string) || req.body.token || req.body.code;
  ...
  try {
    const userResult = await db.select().from(users).where(eq(users.emailVerifyCode, token)).limit(1);
    if (userResult.length === 0) {
      if (req.method === 'GET') {
